package com.educastle.slimademo.web.rest;
...
import com.educastle.slimademo.config.KafkaProperties;
...
class SlimademoKafkaResourceIT {
    ...
    void setup() {
        ...
        SlimademoKafkaResource kafkaResource = new SlimademoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/slimademo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/slimademo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
